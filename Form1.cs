...
namespace HopeForPravilnost
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            var тринадцатый = new Район(0, "Тринадцатый", 3, 2);
            ...
            var московский = new Район(1, "Московский", 2, 3);
                ...
                foreach (var line in lines)
                {
                    ...
                    if (parts.Length >= 2)
                    {
                        if (parts[0].Trim() == "Государство")
                        {
                            ...
                            var государство = new Государство(id, название, годСоздания) { ТекущийПравитель = правитель, ПутьККартинке = путьККартинке };
                            государства.Add(государство);
                        }
                        else if (parts[0].Trim() == "Область")
                        {
                            int id = int.Parse(parts[1].Trim());
                            string название = parts[2].Trim(); // Здесь название области уже без префикса
                            ...
                            var государство = государства.Find(g => g.ID == idГосударства);
                ...
                if (saveFileDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    using (StreamWriter writer = new StreamWriter(filePath))
                    {
                        foreach (var государство in государства)
                        {
                            ...
                            {
                                // Убираем префикс "обл." от названий областей
                                writer.WriteLine($"Область,{область.ID},{область.Название.TrimStart("обл.".ToCharArray())},{государство.ID}");

                                // Сохраняем города
                                foreach (var город in область.Города)
                                {
                                    writer.WriteLine($"Город,{город.ID},{город.Название},{область.ID}"); // добавляем ID области

                                    // Сохраняем районы
                                    foreach (var район in город.Районы)
                                    {
                                        writer.WriteLine($"Район,{район.ID},{район.Название},{город.ID}"); // добавляем ID города
                                    }
                                }
                            }
                        }
